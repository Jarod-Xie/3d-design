...
export class PreviewManager {
  ...
  public async createEngine(id: string, config?: any) {
    ...
    if (this.modelUrl.includes("图标2") || this.modelUrl.includes("图标3")) {
      ...
      this.engine.canvas._webCanvas.addEventListener("mousemove", (e) =>
  ...
  async loadModel(url: string) {
    ...
    // 移除老模型
    if (modelGltf)
      this.rootEntity.removeChild(modelGltf.defaultSceneRoot);

      modelGltf = await this.engine.resourceManager.load<GLTFResource>(url);
    this.renderers_in_modelGltf = new Array<MeshRenderer>();
    modelGltf.defaultSceneRoot.getComponentsIncludeChildren(
      MeshRenderer,
    ...
    this.modelEntity.addChild(modelGltf.defaultSceneRoot);
  }
  ...
  setAction(type: number) {
    ...
    switch (type) {
      ...
      case 2:
        ...
        break;
      case 3:
        actionEntity.addComponent(MouseMove).init(this.engine.canvas._webCanvas);
        break;
      case 4:
        // 关闭模型动画
        let { animations } = modelGltf;
        if (animations && animations?.length > 0) {
          const animator =
            modelGltf.defaultSceneRoot.getComponent(Animator);
          animations?.forEach((clip: AnimationClip, index) => {
            animator.speed = 0;
          });
          actionEntity.addComponent(HoverAnimation).init(this.engine.canvas._webCanvas);
        }
        break;
    }
  }
...
export class MouseMove extends Script {
  ...
  init(webCanvas: HTMLCanvasElement) {
    webCanvas.addEventListener("mouseenter", this.onDocumentMouseEnter);
    webCanvas.addEventListener("mousemove", this.onDocumentMouseMove);
    webCanvas.addEventListener("mouseout", this.onDocumentMouseout);
  }
...
class HoverAnimation extends Script {
  ...
  init(webCanvas: HTMLCanvasElement) {
    webCanvas.addEventListener("mouseover", this.onmouseover);
    webCanvas.addEventListener("mouseout", this.onmouseout);
  ...
  onDestroy(): void {
    this.webCanvas?.removeEventListener("mouseover", this.onmouseover);
    this.webCanvas?.removeEventListener("mouseout", this.onmouseout);
    this.onmouseover();
  }
}
